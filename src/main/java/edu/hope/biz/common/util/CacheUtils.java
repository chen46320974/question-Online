package edu.hope.biz.common.util;

import cn.hutool.cache.*;

/**
 * Cache工具类
 */
public class CacheUtils {



	private static cn.hutool.cache.Cache<String,Object> fifoCache = SpringUtil.getBean(Cache.class);

	public static void put(String key,Object obj){
		fifoCache.put(key,obj);
	}

	public static Object get(String key){
		return fifoCache.get(key);
	}

	
	
//	private static CacheManager cacheManager = SpringUtil.getBean(CacheManager.class);
	/*static{
		EhCacheCacheManager cacheCacheManager= SpringUtils.getBean(EhCacheCacheManager.class);
		cacheManager = cacheCacheManager.getCacheManager();
	}*/


	private static final String SYS_CACHE = "sysCache";

	public static final String CUS_CACHE = "cusCache";

	public static final String CUS_CACHE_ID = "cusId_";

	public static final String USER_CACHE_ID = "userId_";

	public static final String USER_CACHE = "userCache";

//	/**
//	 * 获取SYS_CACHE缓存
//	 * @param key
//	 * @return
//	 */
//	public static Object get(String key) {
//		return get(SYS_CACHE, key);
//	}
//
//	/**
//	 * 写入SYS_CACHE缓存
//	 * @param key
//	 * @return
//	 */
//	public static void put(String key, Object value) {
//		put(SYS_CACHE, key, value);
//	}
//
//	/**
//	 * 从SYS_CACHE缓存中移除
//	 * @param key
//	 * @return
//	 */
//	public static void remove(String key) {
//		remove(SYS_CACHE, key);
//	}
//
//	/**
//	 * 获取缓存
//	 * @param cacheName
//	 * @param key
//	 * @return
//	 */
//	public static Object get(String cacheName, String key) {
//		Element element = getCache(cacheName).get(key);
//		return element==null?null:element.getObjectValue();
//	}
//
//	/**
//	 * 写入缓存
//	 * @param cacheName
//	 * @param key
//	 * @param value
//	 */
//	public static void put(String cacheName, String key, Object value) {
//		Element element = new Element(key, value);
//		getCache(cacheName).put(element);
//	}
//
//	/**
//	 * 从缓存中移除
//	 * @param cacheName
//	 * @param key
//	 */
//	public static void remove(String cacheName, String key) {
//		getCache(cacheName).remove(key);
//	}
//
//	/**
//	 * 获得一个Cache，没有则创建一个。
//	 * @param cacheName
//	 * @return
//	 */
//	private static Cache getCache(String cacheName){
//		Cache cache = cacheManager.getCache(cacheName);
//		if (cache == null){
//			cacheManager.addCache(cacheName);
//			cache = cacheManager.getCache(cacheName);
//			cache.getCacheConfiguration().setEternal(true);
//		}
//		return cache;
//	}
//
//	public static CacheManager getCacheManager() {
//		return cacheManager;
//	}

}
